...
namespace RosTools
{
    ...
    void ROSMarkerPublisher::add(const visualization_msgs::Marker &marker)
    {
        ...
        if (marker.id > max_size_ - 1)
        {
            ...
            ROS_DEBUG_STREAM("ROS VISUALS (topic = " << topic_name_ << "): exceeded max size of " << prev_size << ". New size: " << max_size_);
    ...
    void ROSLine::addLine(const Eigen::Vector2d &p1, const Eigen::Vector2d &p2, double z)
    {
        ...
        addLine(vecToPoint(Eigen::Vector3d(p1(0), p1(1), z)), vecToPoint(Eigen::Vector3d(p2(0), p2(1), z)));
    }
    ...
    {
        addPointMarker(vecToPoint(Eigen::Vector3d(p(0), p(1), z)));
    }
    ...
    void ROSModelMarker::addPointMarker(const Eigen::Vector2d &p, double z)
    {
        addPointMarker(Eigen::Vector3d(p(0), p(1), z));
    }
